...
impl Line2D {
    /// Ensures lexigraphical ordering of points (p <= q) - TODO: should we allow p == q?
    pub fn new(px: f64, py: f64, qx: f64, qy: f64) -> Line2D {
        let mut p = Point2D::new(px, py);
        let mut q = Point2D::new(qx, qy);
        if p > q {
...
mod tests {
    ...
    fn test_slope_vertical() {
        ...
        let vertical_line = Line2D::new(0.0, 0.0, 0.0, -1.0);
        assert_eq!(vertical_line.slope(), f64::INFINITY); // not -INFINITY, since we swap p and q
    ...
    #[test]
    fn test_new_ordering() {
        let line = Line2D::new(1.0, 1.0, 0.0, 0.0);
        assert_eq!(line.p, Point2D::new(0.0, 0.0));
        assert_eq!(line.q, Point2D::new(1.0, 1.0));
    }

    #[test]
    fn test_partial_ord() {
        let line1 = Line2D::new(0.0, 0.0, 1.0, 1.0);
        let line2 = Line2D::new(0.0, 0.0, 2.0, 2.0);
        assert!(line1 < line2);
    }
