...
impl PartialEq for Event {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            (Event::StartEvent { line: line1 }, Event::StartEvent { line: line2 }) => {
                line1 == line2
            }
...
impl Ord for Event {
    fn cmp(&self, other: &Self) -> Ordering {
        match (self, other) {
            (Event::StartEvent { line: line1 }, Event::StartEvent { line: line2 }) => {
                line1.start.cmp(&line2.start)
            }
            (Event::StartEvent { line: start }, Event::EndEvent { line: end }) => {
                start.start.cmp(&end.end)
            }
            (
                Event::StartEvent { line: start },
                Event::IntersectionEvent {
                    intersection: i1, ..
                },
            ) => start.start.cmp(get_point_of_intersection(i1)),
            (Event::EndEvent { line: line1 }, Event::EndEvent { line: line2 }) => {
                line1.end.cmp(&line2.end)
            }
            (Event::EndEvent { line: end }, Event::StartEvent { line: start }) => {
                end.end.cmp(&start.start)
            }
            (
                Event::EndEvent { line: end },
                Event::IntersectionEvent {
                    intersection: i1, ..
                },
            ) => end.end.cmp(get_point_of_intersection(i1)),
            (
                Event::IntersectionEvent {
                    intersection: i1, ..
                },
            ...
            (
                ...
                Event::StartEvent { line: start },
            ) => get_point_of_intersection(i1).cmp(&start.start),
            (
                Event::IntersectionEvent {
                    intersection: i1, ..
                },
                Event::EndEvent { line: end },
            ) => get_point_of_intersection(i1).cmp(&end.end),
        }
    }
}
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Event::IntersectionEvent {
                ...
                write!(
                    f,
                    "IntersectionEvent between at x={}\t| Lines: {} and {}",
                    get_point_of_intersection(intersection).x,
                    smaller,
                    bigger
                )
            }
...
mod tests {
    ...
    fn test_event_equality() {
        ...
        let i1 = Intersection::PartialOverlap {
            line1: l1,
            line2: l2,
            overlap: l1,
        };
        ...
        let ie1 = Event::IntersectionEvent {
            bigger: l1,
            smaller: l2,
            intersection: i1,
        };
        let ie2 = Event::IntersectionEvent {
            bigger: l1,
            smaller: l2,
            intersection: i1,
        };
    ...
    fn test_event_ordering() {
        ...
        let ip = Point2D {
            x: OrderedFloat(1.75),
            y: OrderedFloat(1.5),
        }; // not correct, but doesn't matter for the test

        let i2 = Intersection::Crossing {
            line1: l1,
            line2: l2,
            point: ip,
        };
        ...
        let e7 = Event::IntersectionEvent {
            bigger: l2,
            smaller: l1,
            intersection: i2,
        }; // (1.75, 1.5)
